...
config._reload = function () {
  ...
  if (_.has(process.env, 'CONFIG_PUSH_QUEUE_NAME')) {
    config.pushQueue.name = process.env.CONFIG_PUSH_QUEUE_NAME
  }

  if (_.has(process.env, 'CONFIG_PUSH_QUEUE_WORKER')) {
    config.pushQueue.worker = process.env.CONFIG_PUSH_QUEUE_WORKER === 'true'
    debug('config.pushQueue.worker =', config.pushQueue.worker)
  }
