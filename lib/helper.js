...
helper.prepareApnPayload = function(dataPayload) {
    const apnPayload = {aps: {}};

    if (!_.has(dataPayload, 'notification_html')) {
      return null;
    }
    ...
    if (apnMessage.length === 0) {
      return null;
    }
    apnPayload.aps.alert = apnMessage;
...
helper.prepareApnConnectionOptions = function(packageId, config) {
    ...
    _.forEach(config, function(value, key) {
        ...
        case 'address':
        case 'gateway':
          connectionOptions.address = value;
        break;
        case 'cert':
        case 'cert_data':
    ...
    if (!_.has(connectionOptions, 'cert') &&
      !_.has(connectionOptions, 'key') &&
      !_.has(connectionOptions, 'token')
    ) {
...
helper.prepareWnsPayload = function(dataPayload, extraData) {
    const wnsPayload = _.cloneDeep(dataPayload);
