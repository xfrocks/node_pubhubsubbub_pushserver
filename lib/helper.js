...
helper.prepareFcmPayload = (dataPayload, options = {}) => {
  ...
  const fcmPayload = {}
  const { badgeWithConvo, clickAction, notification } = options
  ...
  const hasUnreadConversation = badgeWithConvo && typeof dataPayload.user_unread_conversation_count !== 'undefined'
  const hasUnreadNotification = typeof dataPayload.user_unread_notification_count !== 'undefined'
  const hasBadgeValue = hasUnreadConversation || hasUnreadNotification

  if (isNotificationMessage) {
    if (hasBadgeValue) {
      let badge = 0
      if (hasUnreadConversation) {
        badge += parseInt(dataPayload.user_unread_conversation_count)
      }
      if (hasUnreadNotification) {
        badge += parseInt(dataPayload.user_unread_notification_count)
      }
      fcmPayload.notification.badge = `${badge}`
    }
...
const prepareHmsDataValue = (targetObj, key, value) => {
  if (_.isObject(value)) {
    targetObj[key] = {}
    Object.keys(value).forEach(k => prepareFcmPayloadValue(targetObj[key], k, value[k]))
  } else {
...
helper.prepareHmsMessage = (dataPayload, options = {}) => {
  ...
  if (typeof dataPayload.notification_id !== 'undefined' &&
    ...
    hmsData.notification_id = `${dataPayload.notification_id}`

    if (typeof notificationDefaultTitle === 'string') {
      hmsAndroid.notification = {
        body,
        tag: `notificationId=${nId}`,
        title: notificationDefaultTitle
      }
      isNotificationMessage = true
    } else {
      hmsData.body = body
  ...
  if (!isNotificationMessage && typeof dataPayload.message === 'object') {
    // prepare convo message
    const {
      creator_username: convoUsername,
      message: convoMessage
  ...
  if (isNotificationMessage) {
    if (typeof badgeClass === 'string' && hasBadgeValue) {
      let badge = 0
      if (hasUnreadConversation) {
        badge += parseInt(dataPayload.user_unread_conversation_count)
      }
      ...
      hmsAndroid.notification.badge = {
        class: badgeClass,
        set_num: badge
      }
    }

    if (typeof hmsAndroid.notification.body === 'string') {
      var clickActionType = 3
      if (typeof clickAction === 'string' || typeof intentPrefix === 'string') {
        clickActionType = 1
      }

      hmsAndroid.notification.click_action = { type: clickActionType }

      if (clickActionType === 1) {
        if (typeof clickAction === 'string') {
          hmsAndroid.notification.click_action.action = clickAction
        }
        if (typeof intentPrefix === 'string') {
          var intent = intentPrefix
          const {
            content_type: intentContentType,
            conversation_id: intentConvoId,
            notification_id: intentNotificationId
          } = dataPayload
          if (typeof intentContentType === 'string') {
            intent = `${intent}&content_type=${intentContentType}`
          }
          if (typeof intentConvoId === 'string') {
            intent = `${intent}&convo_id=${intentConvoId}`
          }
          if (typeof intentNotificationId === 'string') {
            intent = `${intent}&notification_id=${intentNotificationId}`
          }
          hmsAndroid.notification.click_action.intent = intent
        }
      }
    }
  }
  ...
  _.forEach(dataPayload, (value, key) => {
    switch (key) {
      ...
      default:
        ...
        prepareHmsDataValue(hmsData, key, value)
    }
  })

  return {
    data: JSON.stringify(hmsData),
    android: hmsAndroid
  }
}
...
helper.prepareSubscribeData = function (reqBody, reqQuery, requiredKeys) {
  ...
  if (hubUri !== '') {
    try {
      hubUriParsed = new url.URL(hubUri)
    } catch (e) {
      // ignore
    }
  ...
  if (hubTopic === '' && hubUriParsed) {
    hubTopic = hubUriParsed.searchParams.get('hub.topic') || ''
  }
  ...
  if (oauthToken === '' && hubUriParsed) {
    oauthToken = hubUriParsed.searchParams.get('oauth_token') || ''
  }
