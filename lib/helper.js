...
helper.prepareSubscribeData = function (reqBody, requiredKeys) {
  ...
  const hubUriWithoutOauthToken = hubUri.replace(/(\?|&)oauth_token=[^&]+(&|$)/, '$1')
  if (hubUriWithoutOauthToken !== hubUri) {
    debug('prepareSubscribeData', 'removed token from `hub_uri`', hubUri)
    hubUri = hubUriWithoutOauthToken.replace(/(\?|&)$/, '')
  }
