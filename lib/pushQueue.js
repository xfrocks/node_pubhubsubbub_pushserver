...
pushQueue._onJob = function(job, done) {
    ...
    var callback = function(err, result) {
        ...
        info = _.merge({}, info, _.pick(result, _.keys(info)));
    ...
    var tryToDeleteDevice = function(type, id, callback) {
        if (!deviceDb) {
...
pushQueue._onAndroidJob = function(job, callback) {
    ...
    if (_.has(data, 'extra_data.package')) {
      ...
      senderOptions.gcmKey = config.gcm.keys[packageId];
    } else {
      senderOptions.gcmKey = config.gcm.keys[config.gcm.defaultKeyId];
    }
