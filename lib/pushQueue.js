...
pushQueue._onHuaweiJob = function (job, callback) {
  ...
  const {
    ...
    notification_default_title: notificationDefaultTitle,
    app: appId
  } = extraData
  ...
  const hmsMessage = helper.prepareHmsMessage(
    data.payload,
    {
      badgeClass,
      badgeWithConvo: helper.isPositive(badgeWithConvo),
  ...
  db.projects.findConfig('hms', appId, (config) => {
    ...
    return pusher.hms(appId, appSecret, data.device_ids, hmsMessage, done)
  })
}
