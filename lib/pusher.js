...
pusher.setup = function (_apn, _firebaseAdmin, _gcm, _wns) {
  ...
  pusher._hmsPusher = require('./pusher/hms').setup()
...
pusher.stats = function () {
  ...
  if (pusher._hmsPusher) {
    promises.push(pusher._hmsPusher.stats())
  }
...
pusher.hms = function (appId, appSecret, tokens, payload, callback) {
  return pusher._hmsPusher.send(appId, appSecret, tokens, payload, callback)
}
