...
pusher.send = function (projectId, projectConfig, registrationTokens, payloadWithOptions, callback) {
  ...
  const options = {}
  const payload = {}
  Object.keys(payloadWithOptions).forEach(k => {
    if (k === 'data' || k === 'notification') {
      payload[k] = payloadWithOptions[k]
    } else {
      options[k] = payloadWithOptions[k]
    }
  })

  app.messaging().sendToDevice(registrationTokens, payload, options)
    .then((response) => {
