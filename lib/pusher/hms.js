      ...
      return axios.post(
        ...
        (response) => {
          ...
          if (status === 200) {
            ...
            debug('Sent message!', payload, data)
          } else {
            errors.send = { payload, status, data }
            result.failed = token.length
            debug('Unable to send message', status, data)
          }
        },
        (error) => {
          errors.sendRejected = { payload, error }
          result.failed = token.length
          debug('Send message error', error)
        }
      )
    }, (error) => {
      errors.token = { appId, error }
      result.failed = token.length
