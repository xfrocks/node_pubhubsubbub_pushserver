...
admin.setupProjects = function (app, prefix, db) {
  ...
  app.get(prefix + '/projects/apn', function (req, res) {
    const { searchParams } = new url.URL(req.url, 'http://domain.com')
  ...
  app.get(prefix + '/projects/hms', function (req, res) {
    const { searchParams } = new url.URL(req.url, 'http://domain.com')

    return res.render('admin/projects/hms', {
      prefix,
      searchParams
    })
  })

  app.post(prefix + '/projects/hms', function (req, res) {
    const {
      app_id: appId,
    ...
    db.projects.saveHms(appId, appSecret,
      function (isSaved) {
        if (isSaved !== false) {
          debug('POST /projects/hms', 'Saved', appId)
          return res.sendStatus(202)
        } else {
          return res.sendStatus(500)
        }
